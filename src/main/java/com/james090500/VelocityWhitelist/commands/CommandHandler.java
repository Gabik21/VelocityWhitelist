...
import com.james090500.VelocityWhitelist.helpers.UUIDFetcher;
...
public class CommandHandler {
  ...
  public int about(CommandContext<CommandSource> commandSourceCommandContext) {
    ...
    source.sendMessage(LegacyComponentSerializer.legacyAmpersand()
        .deserialize("&a" + velocityWhitelist.PREFIX + "Whitelist is " + status));
    source.sendMessage(LegacyComponentSerializer.legacyAmpersand()
        .deserialize("&a" + velocityWhitelist.PREFIX + "VelocityWhitelist by james090500"));
    return 1;
  }
  ...
  public int turnOn(CommandContext<CommandSource> commandSourceCommandContext) {
    ...
    if (Configs.getConfig().isEnabled()) {
      source.sendMessage(LegacyComponentSerializer.legacyAmpersand()
          .deserialize("&c" + velocityWhitelist.PREFIX + "Whitelist is already turned on"));
    } else {
      Configs.getConfig().setEnabled(true);
      Configs.saveConfig(velocityWhitelist);
      source.sendMessage(LegacyComponentSerializer.legacyAmpersand()
          .deserialize("&a" + velocityWhitelist.PREFIX + "Whitelist turned &2&lON"));
    }
    return 1;
  }
  ...
  public int turnOff(CommandContext<CommandSource> commandSourceCommandContext) {
    ...
    if (!Configs.getConfig().isEnabled()) {
      source.sendMessage(LegacyComponentSerializer.legacyAmpersand()
          .deserialize("&c" + velocityWhitelist.PREFIX + "Whitelist is already turned off"));
    } else {
      Configs.getConfig().setEnabled(false);
      Configs.saveConfig(velocityWhitelist);
      source.sendMessage(LegacyComponentSerializer.legacyAmpersand()
          .deserialize("&a" + velocityWhitelist.PREFIX + "Whitelist turned &c&lOFF"));
    }
    return 1;
  }
  ...
  public int add(CommandContext<CommandSource> commandSourceCommandContext) {
    CommandSource source = commandSourceCommandContext.getSource();
    ParsedArgument<CommandSource, ?> username = commandSourceCommandContext.getArguments()
        .get("username");
    if (username == null) {
      source.sendMessage(LegacyComponentSerializer.legacyAmpersand()
          .deserialize("&c" + velocityWhitelist.PREFIX + "Syntax /vwhitelist add <username>"));
      return 1;
    }
  ...
  public int remove(CommandContext<CommandSource> commandSourceCommandContext) {
    ...
    if (username == null) {
      source.sendMessage(LegacyComponentSerializer.legacyAmpersand()
          .deserialize("&c" + velocityWhitelist.PREFIX + "Syntax /vwhitelist remove <username>"));
      return 1;
    }
  ...
  public int reload(CommandContext<CommandSource> commandSourceCommandContext) {
    ...
    CommandSource source = commandSourceCommandContext.getSource();
    source.sendMessage(LegacyComponentSerializer.legacyAmpersand()
        .deserialize("&a" + velocityWhitelist.PREFIX + "Reloaded"));
    return 1;
  }

  public int list(CommandContext<CommandSource> ctx) {
    String whitelisted = Configs.getWhitelist().stream().map(UUIDFetcher::getName)
        .collect(Collectors.joining(", "));
    ctx.getSource()
        .sendMessage(Component.text("Whitelisted players: " + whitelisted, NamedTextColor.GREEN));
    return 1;
  }
}